...
class BackupService {
  ...
  Future<List<Map<String, dynamic>>> getTodayMatchesWithPredictions() async {
    ...
    final matches = await _supabase
        ...
        .select('''
          ...
          estado
        ''')
        .gte('fecha_hora', startOfDay.toIso8601String())
        .lt('fecha_hora', endOfDay.toIso8601String())
    ...
    for (final match in matchesList) {
      ...
      match['predictions'] = List<Map<String, dynamic>>.from(predictions);
    }
  ...
  Future<Uint8List> buildDailyBackupPdf() async {
    ...
    final matches = await getTodayMatchesWithPredictions();
    final now = DateTime.now();
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          pw.SizedBox(height: 20),
          pw.Text(
            'Partidos y pronósticos del día',
            style: pw.TextStyle(
              fontSize: 16,
              fontWeight: pw.FontWeight.bold,
            ),
          ...
          if (matches.isEmpty)
            pw.Text('No hay partidos programados para hoy.')
          else
            ...matches.expand((match) {
              final predictions =
                ...
                pw.Container(
                  ...
                  child: pw.Column(
                    ...
                    children: [
                      ...
                      pw.Text('Resultado oficial: $marcadorOficial'),
                      pw.SizedBox(height: 8),
                      if (predictions.isEmpty)
                        pw.Text('Sin pronósticos registrados')
                      else
                        pw.Table.fromTextArray(
                          headerStyle:
